...
LOG_PATHS = {'feed api': '/data/logs/nginx/feed/'}
...
def handle_log(log, total_c, total_r):
    ...
    with open(log, 'r') as f:
        for line in f:
            tline = line
	    if tline.find('/api/recommend/feed') < 0:
                pass
            else:
...
def http_post(html):
    url = 'http://10.200.26.28:5000/sendmail'
    log_time = (date.today() - timedelta(1)).strftime("%Y年%m月%d日")
    html = '<p>主机：%s</p><p></p>' % socket.gethostname() + html
    receivers = ['shanghai@cloudin.cn']
    ...
    data = {'smtpserver': {'server': 'smtp.exmail.qq.com', 'mailuser': 'no-reply-devops@cloudin.cn', 'mailpasswd': 'migu1qaz!QAZ'},
            'emailsubject': {'subject': '统计结果：' + log_time + 'feed服务状态统计', 'mess': html},
#            'receivesuser': {'receiver': receivers},
#            'fromuser': {'fromer': fromer}}
            'fromuser': {'fromer': fromer},
            'receivesuser': {'receiver': 'zhangbo@cloudin.cn'}}
